...
def main():
    ...
    try:
        if len(sys.argv) != 6:
            print(
                "Usage: python main.py <input> <output> <model> <scale> <log_path>",
                file=sys.stderr
            )
            sys.exit(1)
        ...
        log_to_file(
            ...
            f"Model: {model_type}, Scale: {requested_scale}"
        )
        ...
        try:
            ...
            if upscaled_4x_image:
                ...
                if requested_scale < 4:
                    ...
                    final_image = upscaled_4x_image.resize(
        ...
        log_to_file(f"FATAL ERROR: Process timed out. {e}")
        log_to_file(traceback.format_exc())
        sys.exit(1)
    except Exception as e:
        log_to_file(f"FATAL ERROR: An unexpected error occurred.")
        log_to_file(traceback.format_exc())
        sys.exit(1)
